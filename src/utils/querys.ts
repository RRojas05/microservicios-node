...
class Querys {
  ...
  constructor() {
    ...
    this.createUserTable = `CREATE TABLE ${config.tables.users} (
    ...
    this.createAuthTable = `CREATE TABLE ${config.tables.auth} (
      id VARCHAR(36) NOT NULL,
    ...
    this.createUserFollowTable = `CREATE TABLE ${config.tables.follow} (
  ...
  public getByField = (table: string, param: string, field: string) => {
    return `SELECT * FROM ${table} WHERE ${field}= '${param}' LIMIT 1`;
  };
  ...
  public delete = (table: string, param: string, field: string) => {
    return `DELETE FROM ${table} WHERE ${field}= '${param}' LIMIT 1`;
  };
